            ...
            <div className="flex gap-1 overflow-x-auto lg:block lg:space-y-0.5 lg:overflow-visible">
              {topLevel.map((t) => (
                <button
                  ...
                  className={`flex shrink-0 items-center gap-2 rounded-md px-2 py-2 text-left text-sm lg:w-full transition-colors ${
                    previewId === t.id
                ...
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-md border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800"
                    >
                      <FileDown size={13} /> Export this doc
                    </button>
                    <Link
                      href={`/tasks/${previewTask.id}?${DOCS_FROM}`}
                      className="rounded-md border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors dark:border-slate-800 dark:text-slate-400 dark:hover:bg-slate-800"
                    >
                      Open task
                    </Link>
                  </div>
                </div>
                <pre className="max-h-[600px] overflow-auto whitespace-pre-wrap rounded-lg bg-slate-50 p-4 font-mono text-xs leading-relaxed text-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
                  {previewDoc}
                </pre>
              </>
            ) : (
